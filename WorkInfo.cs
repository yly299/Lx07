...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public Model.WorkInfo GetModel(int id)
        {
            ...
            {
                string sql = "select workinfo.*,activity.activity.activityName from workinfo join activity on workinfo.activityId=activity.activityId where workId=@id";
        ...
        public int UpdateImg(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                if (workinfo.workPicture == null)
                    sql = "update workinfo set workIntroduct=@workIntroduct where workId=@workId";
                return cn.Execute(sql, workinfo);
            }
        }
